...
const NEUTRAL_TYPES = new Set([
  "inizio", "inizio_secondo_tempo", "fine_primo_tempo", "fine",
  "interruzione", "ripresa", "var",
]);
...
function buildTimelinePayload(detail, homeName, awayName) {
  ...
  for (const e of mapped) {
    if (NEUTRAL_TYPES.has(e.type)) {
      const key = `${e.minute}_${e.type}`;
      const existing = seen.get(key);
      if (!existing || (e.text || "").length > (existing.text || "").length) {
        seen.set(key, { ...e, side: "neutral" });
      }
    } else {
      teamEvents.push(e);
    }
  }
  for (const v of seen.values()) neutral.push(v);
  const all = [...teamEvents, ...neutral];
  all.sort((a, b) => minOf(a.minute) - minOf(b.minute));
  return all.length ? { match_events: JSON.stringify(all) } : {};
}
...
## File 2: `vercel.json` (nella root del repo)

```json
{
  "crons": [
    {
      "path": "/api/sync-to-base44",
      "schedule": "*/3 * * * *"
    }
  ]
}
...
## File 3: `package.json` — assicurati che abbia `@base44/sdk`

Nel `package.json` del repo API aggiungi tra le dependencies:
```json
"@base44/sdk": "^0.8.44"
```
Poi `npm install`.

## Variabili ambiente (Vercel → Settings → Environment Variables)
```
BASE44_APP_ID         = <il tuo app id Base44>
BASE44_ADMIN_EMAIL    = <email admin del sito>
BASE44_ADMIN_PASSWORD = <password admin>
```
